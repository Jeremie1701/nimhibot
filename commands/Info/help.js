...
module.exports = {
  ...
  description: "Trả về tất cả các Lệnh, cho một lệnh cụ thể", //the command description for helpcmd [OPTIONAL]
  ...
  run: async (client, message, args) => {
    try {
      ...
      if (args[0] && args[0].length > 0) {
        ...
        if (cmd.description) embed.addField("**Miêu tả**", `\`${cmd.description}\``);
        if (cmd.aliases) embed.addField("**Bí danh**", `\`${cmd.aliases.map((a) => `${a}`).join("`, `")}\``);
        ...
        if (cmd.usage) {
          ...
          embed.setFooter("Cú pháp: <> = required, [] = optional");
        }
        return message.reply({
        ...
        const embed = new MessageEmbed()
          ...
          .setFooter(`Để xem Thông tin & Mô tả Lệnh, hãy nhập: ${prefix}help [CMD NAME]`, ee.footericon);
